...
import { blogCreateMultipleView } from '../../store/BlogStore';
import { AlertComponent } from '../AlertComponent';

export const BlogMultipleForm = () => {
    ...
    const onSubmitHandler =  (e) => {
        ...
        if (title !== '' && content !== '' && file !== '') {
            // console.log(file)    
            // const blogObj = {title, content, file}
            // console.log(blogObj)
            ...
            const result =  dispatch(blogCreateMultipleView(formData))
            if (result) {
                history.push('/blogs')
            }
